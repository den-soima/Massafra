// App: KendoExceptionReport
var app = angular.module("KendoExceptionReport", ["kendo.directives", "ngDialog", "app.kendoHelper", "app.kendoOds", "app.refresh"]);

// Controller: ExceptionReportCtrl
app.controller("ExceptionReportCtrl", ['$scope', '$interval', '$document', 'ngDialog', 'txt', 'kendoHelper', 'kendoOdsEnumerationTexts', 'refresh', 'kendoOdsMaterials',
function ($scope,$interval, $document, ngDialog, txt, kendoHelper, kendoOdsEnumerationTexts, refresh, kendoOdsMaterials) {
  ...
  $scope.GetColumnNames = function() { return new kendo.data.DataSource({
    ...
    transport: {
      read: {
        url: $("#gatewayPath").data("value") + "odata/ods/ZWebExceptionReportDatas",
    ...
    schema: {
      ...
      if (response.value.length > 0) 
        ...
        for (var i = 0; i < response.value.length; i++) {
          ...
          //Nur für Werte mit Grenzverletzung!
          if (response.value[i].OutOfSpec == 1 || response.value[i].OutOfSpec == 3) {
            var BatchPresent = false;

            m_OOSVCs.push(response.value[i]._ValueCategoryKey);
      ...
      //Werte pivotisieren und Result bilden
        ...
        for (var j = 0, jl = pivotValue.length; j < jl; j++) {

          if ($.inArray(values[i]._ValueCategoryKey, m_OOSVCs) != -1) {
  ...
  $scope.dtExceptionReportEndTime = new Date();
  $scope.dtExceptionReportStartTime = new Date($scope.dtExceptionReportEndTime - 30 * TIMEINTERVAL_PER_DAY_MS);
  ...
  $scope.dateTimePickerExceptionReportStartTime = {
    change: function () {
      $scope.OnFilterChanged();
    }
  };
  ...
  $scope.dateTimePickerExceptionReportEndTime = {
    change: function () {
      $scope.OnFilterChanged();
    }
  };

  // Optionen für Grid ExceptionReportValues        
  $scope.GetGridExceptionReportValuesOptions = function (col, ds) {
    return {
      dataSource: ds,
  ...
  $scope.OnLoadValues = function () {
    ...
    kendoHelper.setDataSourceFilters(m_ColumnNames, "StartTime", "gte", $scope.dtExceptionReportStartTime);
    kendoHelper.setDataSourceFilters(m_ColumnNames, "StartTime", "lte", $scope.dtExceptionReportEndTime);
    ...
    m_dataColumnsInitialized = true;
      ...
      m_checkColumnNames = $interval(function () {
        if (m_ColumnNames._data.length > 0) {
          ...
          for (var i = 0, il = m_ColumnNames._data.length; i < il; i++) {
      ...
      m_checkColumns = $interval(function () {
        if (m_ColumnsInitialized) {
          ...
          // Werte initialisiert
          $scope.gridExceptionReportValues = $scope.GetGridExceptionReportValuesOptions(m_Columns, m_ColumnNames);
    ...
    doMath = function (columnField) {
      var data = $scope.gridExceptionReportValues.dataSource._data;
      var summedValues = 0;
  ...
  $scope.OnFilterChanged = function () {
    ...
    if ($scope.dtExceptionReportStartTime != undefined || $scope.dtExceptionReportEndTime != undefined) {
      kendoHelper.setDataSourceFilters(m_dataSourceFilterBatch, "StartTime", "gte", $scope.dtExceptionReportStartTime);
      kendoHelper.setDataSourceFilters(m_dataSourceFilterBatch, "StartTime", "lte", $scope.dtExceptionReportEndTime);
    }
